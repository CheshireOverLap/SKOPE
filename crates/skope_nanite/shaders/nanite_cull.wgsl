...
// Group 1: Meshlets + triangle data
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Determine which instance this meshlet belongs to.
    // For now: flat dispatch where meshlet_idx maps to a global meshlet array.
    // Instance ID is encoded in the meshlet's group_id (simplification).
    // TODO: proper instance → meshlet mapping via per-mesh meshlet offset table.
    let instance_id = 0u; // Placeholder: will be properly mapped
    ...
    if use_sw {
        let sw_slot = atomicAdd(&counters[2], 1u);
        // SW dispatch: one workgroup per cluster, updated at the end
        // We'll use a separate pass or atomic to set sw_indirect.x
        atomicMax(&sw_indirect.x, sw_slot + 1u);
    } else {
