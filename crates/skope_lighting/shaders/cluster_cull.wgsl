...
fn cull_lights(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Phase 1: Cull lights into a local array to avoid race conditions.
    // Each thread collects its own matches first, then reserves a contiguous
    // range in the global light_indices buffer with a single atomicAdd.
    var local_light_list: array<u32, MAX_LIGHTS_PER_CLUSTER>;
    var local_count = 0u;
    ...
    for (var i = 0u; i < light_count; i++) {
        ...
        let light_dir = light.direction_radius.xyz;
        let spot_cos_outer = light.params0.x;

        let hit = cull_single_light(
            light_pos, light_radius, light_type,
            light_dir, spot_cos_outer,
            tile_x, tile_y, slice
        );

        if (hit) {
            local_light_list[local_count] = i;
            local_count++;
        }
    }

    // Phase 2: Reserve a contiguous range in the global index buffer,
    // then write all local matches into it.
    var base_offset = 0u;
    if (local_count > 0u) {
        base_offset = atomicAdd(&light_counter, local_count);
        for (var j = 0u; j < local_count; j++) {
            light_indices[base_offset + j] = local_light_list[j];
        }
    }

    light_grid[cluster_idx].offset = base_offset;
    light_grid[cluster_idx].count = local_count;
}
