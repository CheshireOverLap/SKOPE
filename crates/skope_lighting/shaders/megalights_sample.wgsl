...
@group(0) @binding(7) var<storage, read> prev_reservoirs: array<Reservoir>;
...
fn sample_lights(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Merge the previous frame's reservoir into the current one using RIS combination.
    // This dramatically improves convergence by reusing prior sampling decisions.
    if (params.temporal_blend > 0.0 && params.frame_index > 0u) {
        let prev = prev_reservoirs[pixel_idx];

        if (prev.sample_count > 0u && prev.selected_weight > 0.0) {
            // Re-evaluate the previous frame's selected light at current surface
            let prev_p_hat = evaluate_light_contribution(
                prev.selected_light, world_pos, normal
            );

            // Clamp the previous reservoir's sample count to avoid unbounded growth
            // (standard temporal reuse clamp, typically 20x the current M)
            let max_prev_M = M * 20u;
            let clamped_prev_M = min(prev.sample_count, max_prev_M);

            // The temporal candidate's RIS weight for merging:
            // w = p_hat_at_current * W_prev * M_prev
            let temporal_weight = prev_p_hat * prev.selected_weight * f32(clamped_prev_M);

            if (temporal_weight > 0.0) {
                let temporal_rand = rand_float(pcg_hash(pixel_seed(pixel, M + 1u)));
                reservoir_update(&reservoir, prev.selected_light, temporal_weight, temporal_rand);
                // reservoir_update already incremented sample_count by 1,
                // so add (clamped_prev_M - 1) to get the correct total.
                reservoir.sample_count += clamped_prev_M - 1u;
            }
        }
    }
