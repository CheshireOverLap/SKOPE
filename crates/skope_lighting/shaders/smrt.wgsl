...
const VSM_FLAG_MAPPED: u32 = 0x10000u; // 1 << 16
...
fn vsm_lookup(shadow_uv: vec2<f32>) -> vec2<f32> {
    ...
    if (page_entry & VSM_FLAG_MAPPED) == 0u {
        return vec2<f32>(-1.0, -1.0); // No allocated page
    }

    // Decode physical page location from page entry
    // Bit layout matches PageTableEntry (Rust side):
    //   [4:0]   physical_x  (0..31)
    //   [9:5]   physical_y  (0..31)
    //   [16]    FLAG_MAPPED
    let phys_x = page_entry & 0x1Fu;
    let phys_y = (page_entry >> 5u) & 0x1Fu;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < max_steps; i = i + 1u) {
        ...
        let shadow_test = textureSampleCompareLevel(vsm_physical_atlas, vsm_comparison_sampler, phys_uv, sample_depth);
