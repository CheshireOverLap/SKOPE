...
pub struct ClusteredLighting {
    ...
    // Group 1: Lights buffer for GPU cull path
...
impl ClusteredLighting {
    pub fn new(
        ...
        let lights_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Cluster Lights Bind Group Layout"),
            entries: &[
        ...
        // Dummy lights buffer (populated when GPU cull path is used)
        let lights_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Cluster Lights Buffer"),
            size: std::mem::size_of::<[f32; 20]>() as u64, // 1 GpuLight = 5 * vec4 = 80 bytes
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });

        let light_count_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Cluster Light Count"),
            size: 16, // uniform requires 16-byte min for some backends
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });

        let lights_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Cluster Lights Bind Group"),
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            ...
            bind_group_layouts: &[&bind_group_layout, &lights_bind_group_layout],
            immediate_size: 0,
        });
