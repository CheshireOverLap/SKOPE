...
pub struct MegaLightsSystem {
    ...
    // Visible light spatial hash for O(1) per-tile light lookup
    pub visible_light_hash: VisibleLightHash,
...
impl MegaLightsSystem {
    pub fn new(
        ...
        // Create visible light spatial hash
        let visible_light_hash = VisibleLightHash::new(device);
    ...
    pub fn classify_tiles(
        ...
        let bind_group = self.classify_pass.create_bind_group(
            ...
            depth_view,
            self.visible_light_hash.hash_table_buffer(),
            self.visible_light_hash.light_index_buffer(),
            self.visible_light_hash.params_buffer(),
        );
    ...
    /// Copy current denoised output and reservoirs to previous frame buffers for temporal reuse
    pub fn swap_history(&mut self, encoder: &mut wgpu::CommandEncoder) {
        // Copy denoised output texture to previous frame texture
        ...
        // Copy current reservoir buffer to previous reservoir buffer for temporal reservoir reuse
        let reservoir_size = (self.width * self.height) as u64
            * std::mem::size_of::<Reservoir>() as u64;
    ...
    /// Get mutable reference to the visible light hash for building each frame
    pub fn visible_light_hash_mut(&mut self) -> &mut VisibleLightHash {
        &mut self.visible_light_hash
    }

    /// Get reference to the visible light hash
    pub fn visible_light_hash(&self) -> &VisibleLightHash {
        &self.visible_light_hash
    }
}
