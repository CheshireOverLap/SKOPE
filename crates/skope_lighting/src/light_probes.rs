...
pub fn project_cubemap_to_sh(
    ...
    for (face_idx, face_data) in cubemap_data.iter().enumerate() {
        ...
        for y in 0..face_size {
            for x in 0..face_size {
                ...
                // Solid angle weight
                let weight = 4.0 / (face_size as f32 * face_size as f32
                    * (1.0 + u * u + v * v).powf(1.5));

                let radiance = face_data[y * face_size + x];

                // Add to SH
                sh.add_directional_light(dir, radiance, weight);
                weight_sum += weight;
            }
        }
    }
