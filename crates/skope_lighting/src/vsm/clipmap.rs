...
impl Clipmap {
    pub fn new(config: ClipmapConfig, page_table_size: u32) -> Self {
        ...
        for i in 0..config.levels {
            ...
            // page_size = 128 pixels per page side
            let page_size = 128u32;
            let texel_size = (half_extent * 2.0) / (rows * page_table_size * page_size) as f32;
        ...
        Self { config, levels, page_table_size }
    }
    ...
    pub fn update_matrices(&mut self, camera_pos: Vec3, light_dir: Vec3) {
        for (i, level) in self.levels.iter_mut().enumerate() {
            ...
            // page_table_texels = rows * page_table_columns * page_size_pixels
            let page_size: f32 = 128.0;
            let page_table_texels = level.page_table_rows as f32 * self.page_table_size as f32 * page_size;
