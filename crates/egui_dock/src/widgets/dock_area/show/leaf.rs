...
impl<Tab> DockArea<'_, Tab> {
    ...
    fn tabs(
        ...
        for tab_index in 0..tabs_len {
            ...
            let (response, title_id) = if is_being_dragged {
                ...
                let (mut response, close_response) = self.tab_title_with_viewer(
                    tabs_ui,
                    &tab_style,
                    id,
                    label,
    ...
    /// Tab title with custom UI support via tab_viewer.tab_title_ui()
    #[allow(clippy::too_many_arguments)]
    fn tab_title_with_viewer(
        &mut self,
        ui: &mut Ui,
        tab_style: &TabStyle,
        id: Id,
        ...
        let _style = fade.unwrap_or_else(|| self.style.as_ref().unwrap());

        // Check if custom UI will be used (without rendering yet)
        let wants_custom_ui = {
            let leaf = self.dock_state[surface_index][node_index]
                .get_leaf_mut()
                .expect("This node must be a leaf");
            // Peek: check if tab_title_ui would return true
            // We need a dummy check - for now assume custom UI is always used if icon_manager has icon
            true // Always try custom UI path
        };

        if wants_custom_ui {
            // First, allocate space and determine tab_rect
            let galley = label.clone().into_galley(ui, None, f32::INFINITY, TextStyle::Button);
            let x_spacing = 8.0;
            let text_width = galley.size().x + 2.0 * x_spacing + 20.0; // +20 for icon
            let minimum_width = tab_style.minimum_width.unwrap_or(0.0).at_least(text_width);
            let tab_width = preferred_width.unwrap_or(0.0).at_least(minimum_width);
            ...
            let tab_style_state = if focused || is_being_dragged {
                if response.has_focus() {
                    &tab_style.focused_with_kb_focus
                } else {
                    &tab_style.focused
            ...
            ui.painter()
                .rect_filled(tab_rect, tab_style_state.corner_radius, tab_style_state.bg_fill);
            let stroke_rect = rect_stroke_box(tab_rect, 1.0);
            ui.painter().rect_stroke(
                stroke_rect,
                tab_style_state.corner_radius,
                Stroke::new(1.0, tab_style_state.outline_color),
                StrokeKind::Inside,
            );
            ...
            if !is_being_dragged {
                ui.painter().hline(
                    RangeInclusive::new(
                        stroke_rect.min.x + f32::max(tab_style_state.corner_radius.sw.into(), 1.5),
                        stroke_rect.max.x - f32::max(tab_style_state.corner_radius.se.into(), 1.5),
                    ),
                    stroke_rect.bottom(),
                    Stroke::new(2.0, tab_style_state.bg_fill),
                );
            }

            // 2. Now render custom UI content on top
            let content_rect = tab_rect.shrink2(vec2(x_spacing, 2.0));
            ...
            let used_custom = {
                let leaf = self.dock_state[surface_index][node_index]
                    .get_leaf_mut()
                    .expect("This node must be a leaf");
                tab_viewer.tab_title_ui(&mut content_ui, &mut leaf.tabs[tab_index.0], tab_style)
            };

            if !used_custom {
                // Fallback: draw text
                let text_pos = Align2::CENTER_CENTER.pos_in_rect(&content_rect) - galley.size() / 2.0;
                ui.painter().add(TextShape::new(text_pos, galley, tab_style_state.text_color));
            }
