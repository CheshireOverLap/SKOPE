...
pub trait TabViewer {
    ...
    /// Custom tab title UI rendering.
    ///
    /// Override this to render custom content (like images) in the tab title.
    /// Return `true` if you handled the rendering, `false` to use default title text.
    ///
    /// # Example
    /// ```ignore
    /// fn tab_title_ui(&mut self, ui: &mut Ui, tab: &mut Self::Tab, _style: &TabStyle) -> bool {
    ///     ui.horizontal(|ui| {
    ///         ui.image(my_icon);
    ///         ui.label(tab.title());
    ///     });
    ...
    /// }
    /// ```
    fn tab_title_ui(&mut self, _ui: &mut Ui, _tab: &mut Self::Tab, _style: &TabStyle) -> bool {
        false // Use default title rendering
    }
