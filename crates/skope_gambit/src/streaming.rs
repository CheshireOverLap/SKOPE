...
pub struct NaniteStreamingPipeline {
    ...
    pub request_buffer: wgpu::Buffer,
    pub request_count_buffer: wgpu::Buffer,

    // Feedback compute
    pub feedback_pipeline: wgpu::ComputePipeline,
...
impl NaniteStreamingPipeline {
    ...
    pub fn new(device: &wgpu::Device, config: NaniteStreamingConfig) -> Self {
        ...
        // ── Request buffer (GPU streaming requests) ─────────────────
        let max_requests = config.max_requests_per_frame as u64;
        let request_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Nanite Streaming Requests"),
            size: std::mem::size_of::<StreamingRequest>() as u64 * max_requests,
            usage: wgpu::BufferUsages::STORAGE
                | wgpu::BufferUsages::COPY_SRC,
            mapped_at_creation: false,
        });
        ...
        let request_count_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Nanite Streaming Request Count"),
            size: 4, // single u32
            usage: wgpu::BufferUsages::STORAGE
                | wgpu::BufferUsages::COPY_SRC
                | wgpu::BufferUsages::COPY_DST,
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Nanite Streaming Feedback Shader"),
            source: wgpu::ShaderSource::Wgsl(
                include_str!("../shaders/nanite_streaming_feedback.wgsl").into(),
            ),
        });

        // ── Pipeline ────────────────────────────────────────────────
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Nanite Streaming Feedback Pipeline Layout"),
            bind_group_layouts: &[&feedback_layout],
            immediate_size: 0,
        });

        let feedback_pipeline =
            device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                label: Some("Nanite Streaming Feedback Pipeline"),
                layout: Some(&pipeline_layout),
                module: &shader,
                entry_point: Some("reset_feedback"),
    ...
    pub fn check_overflow(&mut self, feedback: &StreamingFeedback, current_time: f64) {
        let capacity = self.config.max_resident_pages;
        ...
        let visible_overflow = feedback.visible_cluster_peak > capacity;
        let node_overflow = feedback.node_peak > capacity;
        ...
        if visible_overflow || node_overflow {
            ...
            let overflow_ratio = worst_peak as f32 / capacity as f32;
            let bias_increment = (overflow_ratio - 1.0).clamp(0.1, 1.0);
            self.lod_bias_adjustment += bias_increment;

            log::debug!(
                "Nanite streaming: overflow detected (visible={}, node={}, capacity={}), ratio={:.2}, bias={:.2}",
