...
pub fn generate_sss_lut_for_type(size: u32, lut_type: SssLutType) -> Vec<[f32; 4]> {
    ...
    for y in 0..size {
        for x in 0..size {
            let n_dot_l = (x as f32 / (size - 1) as f32) * 2.0 - 1.0;
            let curvature = y as f32 / (size - 1) as f32;
...
// ============ GPU Functions (requires "gpu" feature) ============
