...
fn mesh_main(
    ...
    if lid < meshlet.triangle_count {
        ...
        // V-Buffer triangle ID: vc_idx(20) | triangle_id(7) | material_id(5)
        // Store visible_cluster index (not meshlet_id) so material_eval
        // can look up both meshlet_id and instance_id through visible_clusters[].
        output.prims[lid].triangle_id =
            ((cluster_idx & 0xFFFFFu) << 12u)
            | ((lid & 0x7Fu) << 5u)
