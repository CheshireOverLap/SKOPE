...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Read albedo for diffuse modulation
    let uv = vec2<f32>(f32(px) + 0.5, f32(py) + 0.5) / vec2<f32>(f32(params.screen_width), f32(params.screen_height));
    let albedo = textureSampleLevel(albedo_texture, albedo_sampler, uv, 0.0).rgb;

    // Read current HDR color
    let current = textureLoad(hdr_buffer, vec2<i32>(i32(px), i32(py)));

    // Add GI contribution: irradiance * albedo / PI * intensity
    let gi = irradiance * albedo * (1.0 / 3.14159265) * params.gi_intensity;
