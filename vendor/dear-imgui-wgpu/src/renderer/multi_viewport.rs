...
pub unsafe extern "C" fn renderer_create_window(vp: *mut Viewport) {
    ...
    if ptr_val < 0x10000 {
        eprintln!("[wgpu-mv] Renderer_CreateWindow: invalid window_ptr: 0x{:x}", ptr_val);
        return;
    }
    ...
    let config = {
        ...
        if let Some(adapter) = &global.adapter {
            ...
            // Multi-Viewport 성능: Mailbox > Immediate > Fifo (VSync는 각 뷰포트마다 블로킹)
            ...
            // Immediate for better multi-viewport performance
            wgpu::SurfaceConfiguration {
                usage: wgpu::TextureUsages::RENDER_ATTACHMENT,
...
pub unsafe extern "C" fn renderer_render_window(vp: *mut Viewport, _render_arg: *mut c_void) {
    ...
    if let Some(data) = unsafe { viewport_user_data_mut(vp) } {
        ...
        // Debug: 보조 뷰포트 렌더링 정보 로깅
        let disp = draw_data.display_size();
        let disp_pos = draw_data.display_pos();
        let fb_scale = draw_data.framebuffer_scale();
        let draw_list_count = draw_data.draw_lists().count();
        let total_vtx: usize = draw_data.draw_lists().map(|dl| dl.vtx_buffer().len()).sum();
        let total_idx: usize = draw_data.draw_lists().map(|dl| dl.idx_buffer().len()).sum();
        eprintln!("[wgpu-mv] Secondary VP: fb={}x{}, pos=({:.0},{:.0}), disp=({:.0},{:.0}), scale=({:.2},{:.2}), lists={}, vtx={}, idx={}",
            fb_w, fb_h, disp_pos[0], disp_pos[1], disp[0], disp[1], fb_scale[0], fb_scale[1],
            draw_list_count, total_vtx, total_idx);
        ...
        let render_block = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            });
            {
                ...
                // Render secondary viewport with its own frame resources
                // advance_frame=true to avoid vertex/index buffer conflicts with main viewport
                if let Err(e) = renderer.render_draw_data_with_fb_size_ex(
                    &draw_data,
                    &mut render_pass,
                    fb_w,
                    fb_h,
                    true,  // Use separate frame resources for secondary viewports
                ) {
