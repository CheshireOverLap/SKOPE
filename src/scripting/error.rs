...
fn extract_stack_trace(err: &mlua::Error) -> Vec<StackFrame> {
    ...
    // Extract traceback from error message if available
    let err_str = format!("{:?}", err);

    // Parse lines like: "[string "..."]:10: in function 'foo'"
...
fn extract_line_number(message: &str) -> Option<u32> {
    // Common patterns: ":123:" or "line 123"
    let patterns = [
...
mod tests {
    ...
    fn test_error_formatting() {
        ...
        assert!(console.contains("attempt to call nil value"));
    }
