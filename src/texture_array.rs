...
impl TextureArrayManager {
    ...
    fn create_texture_array(
        ...
        if indices.is_empty() {
            // 기본 텍스처 (1x1)
            Self::write_fallback_layer(queue, &texture, 0, size, fallback_color);
        } else {
            for (layer, &tex_idx) in indices.iter().enumerate() {
                if let Some(tex_data) = textures.get(tex_idx) {
                    Self::write_texture_layer(
                        queue,
                        &texture,
                        layer as u32,
                        tex_data,
                    ...
                    Self::write_fallback_layer(queue, &texture, layer as u32, size, fallback_color);
                }
            }
        }
